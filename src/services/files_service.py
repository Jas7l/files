...
class FilesService:
    ...
    def sync_storage_and_db(self):
        """Синхронизация хранилища и БД"""

        local_files_path = {}
        for root, _, files in os.walk(self._st):
            for file in files:
                full_path = os.path.normpath(os.path.join(root, file))
                local_files_path[full_path] = file

        with self._pg.begin():
            db_files = self._pg.query(File).all()
            db_files_path = {
                os.path.normpath(
                    os.path.join(file.path, f'{file.name}.{file.extension}')
                ): file for file in db_files
            }

        files_for_delete = [
            file for path, file in db_files_path.items()
            if path not in local_files_path
        ]

        files_for_upload = []
        for path in local_files_path:
            if path not in db_files_path:
                ...
                files_for_upload.append(new_file)
        ...
        for root, _, _ in os.walk(self._st, topdown=False):
            if (
                    not os.listdir(root)
                    and os.path.abspath(root) != os.path.abspath(self._st)
            ):
                try:
                    os.rmdir(root)
                except OSError:
    ...
    def get_file_by_name(self, file_name: str) -> Dict[str, Any]:
        ...
        with self._pg.begin():
            ...
            file = self._pg.query(File).filter(
                File.name == name,
    ...
    def download_file(self, file_id: int):
        ...
        file_path = os.path.join(
            file.get('path'),
            f'{file.get('name')}.{file.get('extension')}',
        )
    ...
    def upload_file(self) -> Dict[str, Any]:
        """Загрузка файла в хранилище и добавление записи о нём в БД"""

        raw_fields = request.form.get('fields')
        if not raw_fields:
            raise ModuleException(
                'Missing form fields', {'data': ''}, 400
            )
    ...
    def update_file(self, file_id) -> Dict[str, Any]:
        """Обновление файла и записи о нём в БД"""
        ...
        with self._pg.begin():
            ...
            old_path = os.path.join(
                file.path, f'{file.name}.{file.extension}'
            )
            ...
            self._logger.debug(
                'Файл успешно обнавлён', extra={'id': file_id}
            )
            return file.dump()

    def delete_file(self, file_id: int) -> Dict[str, Any]:
        """Удаление файла и записи о нём в БД"""
        ...
        with self._pg.begin():
            ...
            self._pg.delete(file)
            self._logger.debug(
                'Файл успешно удалён', extra={'id': file_id}
            )
            return file.dump()
